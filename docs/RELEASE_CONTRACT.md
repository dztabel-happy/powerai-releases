# PowerAI 发布契约

## 1. 发布边界

- 本仓库只公开安装包、自动更新元数据和发行说明。
- 源代码和发布凭据不得写入 Git 历史、制品或日志。

## 2. 版本规则

- 使用 SemVer，应用版本、Git tag 和更新元数据版本必须一致。
- 正式通道使用 `vX.Y.Z`；预发布版本使用 `vX.Y.Z-beta.N`。
- 已发布版本及其制品不可覆盖或重新上传；修复必须提升版本号。

## 3. 必需制品

当前发布通道仅提供 Windows x64 内测版。Windows 安装包暂不签名，Release 说明必须明确标注；macOS 构建、公证和发布均暂停，不进入当前流水线。

### macOS（暂停）

- `PowerAI-X.Y.Z-mac-arm64.dmg`
- `PowerAI-X.Y.Z-mac-arm64.zip`
- 对应 blockmap
- `latest-arm64-mac.yml`
- 兼容通道需要时发布 `latest-mac.yml`

### Windows（当前）

- `PowerAI-X.Y.Z-win-x64.exe`
- 对应 blockmap
- `latest.yml`
- 发布 ARM64 时增加安装包、blockmap 与 `latest-win-arm64.yml`

## 4. 原子发布

1. 从固定且已合并的源代码提交构建。
2. 完成测试、适用于当前平台的签名要求和安装态验收。
3. 创建 Draft Release 并上传全部制品。
4. 校验文件名、版本、大小、摘要和更新元数据引用。
5. 临时发布为最新版本并立即执行上一公开版本到新版本的真实安装升级验收。
6. 验收失败或验收任务取消时，由独立兜底任务删除新 Release 和标签，并恢复上一版本为 latest；验收通过后发布完成。

禁止先发布更新元数据、后补安装包；否则客户端可能发现一个无法下载的版本。

生产更新器无法读取 Draft Release，因此候选版在验收期间会短暂成为 latest。此时已主动安装候选版的客户端不会被自动降级；验收失败后必须发布版本号更高的修复版。整个工作流被人工取消、GitHub 服务中断或 runner 丢失时，兜底任务仍可能无法执行，操作员必须核对 Release 与 latest 指向。

## 5. 客户端体验

- 后台检查到新版本后，在侧栏“设置”右侧显示蓝色“更新”按钮，不强制弹窗。
- 用户点击后开始下载并显示进度。
- 下载完成后按钮变为“重启更新”；用户确认后退出并安装。
- “设置 → 关于 → 检查更新”保留为手动入口。
- 更新失败必须保留当前可用版本，并向用户显示可重试错误。

## 6. 发布验收

必须使用已经安装的旧版本验证：

1. 发现新版本；
2. 下载进度与状态正确；
3. 重启并完成安装；
4. 应用版本已提升；
5. 用户会话和设置完整；
6. 更新失败不会造成重启循环或不可启动状态。

Windows 内部测试版本必须完成上述升级验收。macOS 恢复发布前，必须重新启用 Developer ID 签名、公证、Gatekeeper 和完整旧版到新版升级验收，不得复用 Windows-only 发布结论。

macOS 暂停期间只允许在开发者 Mac 上本地构建和测试，不生成公开制品，也不进入公开 CI。

## 7. 回滚

不覆盖旧 Release。出现问题时停止当前版本分发并发布版本号更高的修复版本；客户端不得静默降级。
